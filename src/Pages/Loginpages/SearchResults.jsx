...
function Searchresults() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto font-mono">
      <h2 className="text-4xl  mb-4">showing results for <span className="text-red-500">{query} </span>:</h2>
        ...
        results.map((blog) => (
          <Link to={`/medium2/readvlog/${blog._id}`} key={blog._id}>
            <div className="mb-6 py-4 border-b ">
              ...
              <p className="text-sm text-gray-600">by {blog.author?.username || "Unknown"} <span> on </span> {new Date(blog.createdAt).toLocaleDateString()}</p>
